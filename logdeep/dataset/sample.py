...
def session_window(data_dir,datatype,sample_ratio=1):
    ...
    for i in tqdm(range(len(train_df))):
        ...
        padding_seq = trp(ori_seq,50)
        sem_vec = []
        for event in padding_seq:
            if event == 0:
                sem_vec.append([-1]*300)
            else:
                sem_vec.append(event2semantic_vec[str(event-1)])
        result_logs['Semantics'].append(sem_vec)
